...
async function run() {
  ...
  const bgLight = tokens.background.light;
  const pairs = [];
  ...
  // chips / overlay example: rgba white 0.03 over bg
  ...
  // suggest tweaks for failing small text: try darken fg until ratio>=4.5
...
function suggestDarker(fg, bg, target){
  // convert fg to HSL, reduce lightness until contrast >= target or min reached
